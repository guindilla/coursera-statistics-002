...
df$infr <- cut(df$infr, breaks=seq(0, 1, by = 0.2), 
    labels=c("very low","low","medium","high","very high"), include.lowest=TRUE)
table(df$infr)
df$telco <- cut(df$telco, breaks=seq(0, 1, by = 0.2),
    labels=c("very low","low","medium","high","very high"), include.lowest=TRUE)
table(df$telco)

# TMP WORKING
df2 <- df
df2$infr <- cut(df$infr, breaks=seq(0, 1, by = 0.1), 
    labels=c("0-10%","11-20%","21-30%","31-40%","41-50%","51-60%","61-70%","71-80%","81-90%","91-100%"), include.lowest=TRUE)
table(df2$infr)
df2$telco <- cut(df$telco, breaks=seq(0, 1, by = 0.1),
    labels=c("0-10%","11-20%","21-30%","31-40%","41-50%","51-60%","61-70%","71-80%","81-90%","91-100%"), include.lowest=TRUE)
table(df2$telco)
...
Both variables can take the values of *very low* (0-19%),*low* (20-39%),*medium* (40-59%),*high* (60-79%),*very high*(80-100%).
...
mosaic(infr ~ telco,
       ...
       labeling_args = list(rot_labels = c(left = 0),
                            offset_varnames = c(left = 2.5, top = 1),
                            offset_labels = c(left = 1.2, top = 0.5),
